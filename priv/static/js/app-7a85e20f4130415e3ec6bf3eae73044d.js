...
var channel = socket.channel("update:grid", {});
var stepCounter = $('#stepCounter');
var debug = $('#debug');
var peopleConnected = $('#peopleConnected');
...
function handleCellAddition(event) {
    if (mouseIsClicked) {
        var xPosition = Math.ceil(event.offsetX / event.currentTarget.offsetWidth * event.currentTarget.width) - 1;

        var yPosition = Math.ceil(event.offsetY / event.currentTarget.offsetHeight * event.currentTarget.height) - 1;
...
channel.on("new_update", function (payload) {
    ...
    var imageData = context.createImageData(payload.width, payload.height);

    var byteCharacters = atob(payload.game_state);
    ...
    for (var i = 0; i < imageData.data.length; i = i + 4) {
        ...
        var byteIndex = Math.floor(i / 4 / 8);
        var byteToCheck = byteArray[byteIndex];

        var insideByteIndex = i / 4 % 8;
        var insideByteOffset = 0x80 >> insideByteIndex;

        var offOrOn = byteToCheck & insideByteOffset;
